...
namespace NightFlux.Imports
{
    public class NightscoutImporter
    {
        ...
        public static async Task Run(Configuration configuration)
        {
            ...
            await Task.WhenAll(
                ImportBg(nsql)
                //ImportBasalProfiles(nsql),
                //ImportTempBasals(nsql),
                //ImportBoluses(nsql),
                //ImportCarbs(nsql)
            );
        }
        private static async Task ImportBg(NightSql nsql)
        {
            ...
            while (await cursor.MoveNextAsync())
            {
                foreach (BsonDocument document in cursor.Current)
                {
                    ...
                    {
                        await nsql.ImportEntity(new BgValue
                        {
                            Value = gv.Value,
                            Time = dt.Value
                        });
        ...
        //private static async Task ImportBasalProfiles(NightSql nsql)
        //{
        //    try
        //    {
        ...
        //private static async Task ImportTempBasals(NightSql nsql)
        //{
        //    using var cursor = await MongoDatabase.GetCollection<NsTreatment>("treatments")
        ...
        //private static async Task ImportCarbs(NightSql nsql)
        //{
        //    using var cursor = await MongoDatabase.GetCollection<NsTreatment>("treatments")
        ...
        //private static async Task ImportBoluses(NightSql nsql)
        //{
        //    using var cursor = await MongoDatabase.GetCollection<NsTreatment>("treatments")
        ...
        // }
        //
        // private static async Task<BasalProfile?> ParseProfileSwitch(NsTreatment treatment)
        // {
        ...
        //     return ret;
        // }
        // private static async Task<double[]> GetBasalRates(JObject joProfile, int percentage)
        // {
        ...
        // }
        //
        // private static double?[] GetSingleRates(JObject joProfile, int percentage)
        // {
